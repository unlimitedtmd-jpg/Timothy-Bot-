...
const inconnuboyConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from Diana Tech! 🔥' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [DIANA-XMD] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from Diana Tech! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [DIANA-XMD] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
